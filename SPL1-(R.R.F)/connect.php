<?php
  ...
  function sendMail($email,$code)
  {
    ...
    $mail->Body = "Dear user, Thank you for registering. Click the link to verify your email
                   <a href='http://localhost/UserSignUp/verify.php?email=$email&code=$code'> Verify </a>";
  ...
  if(!empty($username) && !empty($location) && !empty($email) && !empty($password) && !empty($confirm))
  {
    if($password == $confirm)
    {
      ...
      else
      {
        ...
        $INSERT= "INSERT Into accounts (username, location, email, password, code, status) values ('$username','$location','$email', '$password', '$code', 0)";
        ...
        if($INSERT && sendMail($email,$code))
        {
          ...
          echo "Verification mail sent. Please verify your email before logging in. Make sure to check your spam folder!";
        }
