<?php
  ...
  function sendMail($useremail,$code)
  {
    ...
    $mail->Body = "Dear user, Thank you for registering. Click the link to verify your email
                   <a href='http://localhost/SPL1--R.R.F-1/SPL1-(R.R.F)/customer/customerverify.php?email=$email&code=$code'> Verify </a>";
  ...
  if(!empty($username) && !empty($ulocation) && !empty($useremail) && !empty($password) && !empty($confirm))
  {
    if (!filter_var($_POST['useremail'], FILTER_VALIDATE_EMAIL)) {
      ...
      header("Location:popup/popup10.html");
      exit();
    ...
    if (strlen($_POST['password']) > 15 || strlen($_POST['password']) < 8  || ctype_upper($_POST['password']) || ctype_lower($_POST['password']) || !preg_match("/[0-9]/", $_POST['password'])) {
      header("Location:popup/popup11.html");
      exit();
  ...
  if(mysqli_num_rows($res) > 0){
    header("Location:popup/popup12.html");
      exit();
      ...
      else
      {
       ...
       // $password = password_hash($_POST['password'], PASSWORD_DEFAULT);
        ...
        {
          header("Location:popup/popup13.html");
          exit();
    ...
    else
    {
      ...
      header("Location:popup/popup14.html");
      exit();
  ...
  else
  {
    ...
    header("Location:popup/popup15.html");
      exit();
